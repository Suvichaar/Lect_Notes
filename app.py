...
uploaded = st.file_uploader("1. Upload your notes image:", type=["png", "jpg", "jpeg"])
if not uploaded:
...
template_uploaded = st.file_uploader("2. Upload HTML template (.html file):", type=["html"])
if not template_uploaded:
    st.info("Please upload your HTML template (e.g., template-v18.html). You can download a sample from the repo.")
    st.stop()
template = template_uploaded.read().decode("utf-8")
...
final_html = fill_template(template, story)
